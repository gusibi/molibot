...
批准计划在当前 Session 的下一轮执行，工具活动、结果与后续反馈保留在同一对话。计划不拥有独立对话；无论从 Session 内嵌进度还是桌面「计划」看板查看，都读取同一份持久化执行记录。

桌面「计划」是工作区导航下的一个目的地，不是聊天 Inspector 的又一适配器：左列是可搜索、按状态筛选的计划列表，右侧是流内 detail 面板（沿用 Providers/Tasks 的 master–detail 轨道与共享语义 token），窄窗收起列表并保留返回入口。计划是独立于聊天的持久化对象；稳定 id 不随改名、修订或换 Session 改变。详情按执行顺序展示两层任务/步骤、真实进度、验收要求与固定操作区，操作按当前状态启用（开始、暂停、继续、取消、删除、返工新增），已完成步骤只读。只保存不执行；返工以新增步骤的新版本表达并回到待批准。

首次批准在计划保存为 Durable Execution 后进行：绑定当前内容版本、按接受时的编辑生成新版本，并交给共享执行器；执行器**在来源 Session 继续该计划的下一轮**，LLM 调用、工具活动与产出仍留在同一对话，计划本身不拥有独立对话。只有没有来源 Session 的执行（定时/项目任务）才使用独立任务归档。跨 Session 继续与执行权认领由共享执行层核验，看板只展示、编辑并控制已经存在的计划。
