...
import { readWebSearchConfig, updateWebSearchConfig } from "$lib/server/settings/handlers/mediaGenerates";

export const GET: RequestHandler = async () => {
  try {
    const value = readWebSearchConfig(getRuntime());
...
export const PUT: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const value = updateWebSearchConfig(getRuntime(), raw);
    return json({ ok: true, value }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
