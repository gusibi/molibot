...
import { listExternalSessionMetaFromContexts } from "$lib/server/app/externalSessionsFromContexts";
import type { DesktopExternalSessionsResponse } from "$lib/shared/desktop";

export const GET: RequestHandler = async () => {
  const sessions = listExternalSessionMetaFromContexts(resolve(config.dataDir));
