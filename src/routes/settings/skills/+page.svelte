  ...
  <div class="grid h-full grid-cols-1 lg:grid-cols-[260px_1fr]">
    <aside class="hidden border-r border-white/10 bg-[#171717] p-3 lg:block">
      <nav class="space-y-1 text-sm">
        ...
        <a class="block rounded-lg bg-white/15 px-3 py-2 font-medium text-white" href="/settings/skills">Skills</a>
      ...
      <div class="mx-auto max-w-5xl space-y-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p class="text-sm text-slate-400">Inspect installed skills by scope and path.</p>
          </div>
          <button
        ...
        {#if loading}
          <div class="rounded-xl border border-white/15 bg-[#2b2b2b] px-4 py-3 text-sm text-slate-300">Loading skills...</div>
        {:else}
          <section class="grid gap-3 rounded-xl border border-white/15 bg-[#2b2b2b] p-4 text-sm text-slate-300 sm:grid-cols-2">
            ...
            <div><span class="text-slate-400">Global skills dir:</span> {globalSkillsDir || "(unknown)"}</div>
            <div><span class="text-slate-400">Global skills:</span> {count.global}</div>
            <div><span class="text-slate-400">Chat skills:</span> {count.chat}</div>
            <div><span class="text-slate-400">Workspace legacy skills:</span> {count.workspaceLegacy}</div>
          ...
          <section class="space-y-3">
            ...
            {#if byScope("global").length === 0}
              <div class="rounded-xl border border-white/15 bg-[#2b2b2b] px-4 py-3 text-sm text-slate-300">No global skills found.</div>
            {:else}
          ...
          <section class="space-y-3">
            ...
            {#if byScope("chat").length === 0}
              <div class="rounded-xl border border-white/15 bg-[#2b2b2b] px-4 py-3 text-sm text-slate-300">No chat-level skills found.</div>
            {:else}
          ...
          <section class="space-y-3">
            ...
            {#if byScope("workspace-legacy").length === 0}
              <div class="rounded-xl border border-white/15 bg-[#2b2b2b] px-4 py-3 text-sm text-slate-300">No workspace-legacy skills found.</div>
            {:else}
