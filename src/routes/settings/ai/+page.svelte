<script lang="ts">
      ...
      const normalized: ProviderModelForm =
        typeof (m as unknown) === "string"
          ...
          : {
              ...
              tags: Array.isArray((m as ProviderModelForm).tags)
                ? (m as ProviderModelForm).tags
                : ["text"],
              supportedRoles: Array.isArray(
  ...
  function ensureDefaultCustomProvider(): void {
    ...
    if (
      !form.customProviders.some((p) => p.id === form.defaultCustomProviderId)
    ) {
      ...
      updateProviderById(providerId, (current) => ({
        ...
        models: current.models.map((m) =>
          m.id.trim() === targetModel
            ? { ...m, supportedRoles: data.supportedRoles }
            : m,
        ),
      }));
          ...
          models: Array.isArray(cp.models)
            ? cp.models.map((m: unknown) => {
                ...
                const tags = Array.isArray(obj.tags)
                  ? obj.tags
                      .map((t) => String(t) as ModelCapabilityTag)
                      .filter((t) => capabilityTags.includes(t))
                  : ["text"];
    ...
    <section class="min-h-0 overflow-y-auto px-4 py-6 sm:px-8">
      <div class="mx-auto max-w-6xl space-y-4">
        ...
        <p class="text-sm text-slate-400">
          Configure providers, model tags, and routing by capability.
        </p>
        ...
        {:else}
          <form class="space-y-4" on:submit|preventDefault={save}>
            <section class="rounded-xl border border-white/15 bg-[#2b2b2b] p-4">
              <div class="grid gap-3 md:grid-cols-2">
                <label class="grid gap-1.5 text-sm">
                  ...
                  <select
                    class="rounded-lg border border-white/15 bg-[#1f1f1f] px-3 py-2 outline-none focus:border-emerald-400"
                ...
                <label class="grid gap-1.5 text-sm md:col-span-2">
                  ...
                  <textarea
                    class="min-h-24 rounded-lg border border-white/15 bg-[#1f1f1f] px-3 py-2 outline-none focus:border-emerald-400"
                    rows="3"
            ...
            <section
              class="grid min-h-[560px] gap-0 overflow-hidden rounded-xl border border-white/15 bg-[#2b2b2b] lg:grid-cols-[300px_1fr]"
            >
              <aside
                class="border-b border-white/10 p-4 lg:border-b-0 lg:border-r"
              >
                ...
                <div class="mt-3 space-y-2">
                  ...
                  {#each filteredCustomProviders() as provider (provider.id)}
                    <button
                      type="button"
                      class={`w-full cursor-pointer rounded-lg border px-3 py-2 text-left text-sm transition-colors duration-200 ${
                        selectedProviderId === provider.id
                ...
                {#if getSelectedProvider()}
                  ...
                  <section class="mt-5 space-y-3 border-t border-white/10 pt-4">
                    ...
                    {#each cp.models as model, index}
                      <div
                        class="rounded-lg border border-white/10 bg-[#252525] p-3"
                      >
                        <div class="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
                          ...
                          <button
                            ...
                            disabled={!model.id.trim() ||
                              testingModelKey === `${cp.id}|${model.id.trim()}`}
                          >
                            {testingModelKey === `${cp.id}|${model.id.trim()}`
                              ? "Testing..."
                              : "Test"}
                        ...
                        <div class="mt-2 flex flex-wrap gap-2">
                          {#each capabilityTags as tag}
                            <label
                              class="inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-[#1f1f1f] px-2 py-1 text-xs"
                            >
                              <input
                                type="checkbox"
                                checked={model.tags.includes(tag)}
                                on:change={() => toggleTag(cp.id, index, tag)}
                              />
                              <span>{tag}</span>
                            </label>
                          {/each}
                  ...
                  <section class="mt-5 space-y-2 border-t border-white/10 pt-4">
                    ...
                    {:else}
                      {#each cp.models as model}
                        {#if model.id.trim()}
                          <div
                            class="flex flex-col gap-1 rounded-lg border border-white/10 bg-[#252525] px-3 py-2 sm:flex-row sm:items-center sm:justify-between"
                          >
                            <strong class="text-sm">{model.id}</strong>
                            <div class="flex flex-wrap gap-1.5">
                              {#each model.supportedRoles as role}
                                <span
                                  class="rounded-full border border-white/20 px-2 py-0.5 text-xs text-slate-300"
                                  >{role}</span
                                >
                              {/each}
                            </div>
                    ...
                    <p class="text-xs text-slate-500">
                      Use each model's Test button to refresh its supported
                      roles.
                    </p>
                  </section>
                {:else}
                  <p class="text-sm text-slate-500">
                    Select a provider from the left list.
                  </p>
                {/if}
              </div>
            </section>
