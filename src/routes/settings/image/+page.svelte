<script lang="ts">
  ...
  const engines: Array<{ id: EngineId; name: string; hint: string; keyLabel: string; defaultUrl: string }> = [
    { id: "agnes", name: "Agnes Image", hint: "High-performance OpenAI-compatible editing and generation (agnes-image-2.0-flash). ELO 1,184.", keyLabel: "AGNES_API_KEY", defaultUrl: "https://apihub.agnes-ai.com" },
    { id: "google", name: "Google Imagen", hint: "Excellent English instruction adherence, realism, and style consistency.", keyLabel: "GOOGLE_API_KEY", defaultUrl: "https://generativelanguage.googleapis.com" },
    { id: "volcengine", name: "Volcengine (Seedream)", hint: "Outstanding Chinese comprehension and illustration generation.", keyLabel: "VOLCENGINE_API_KEY", defaultUrl: "https://ark.cn-beijing.volces.com" },
    { id: "modelscope", name: "ModelScope", hint: "High-speed and cost-effective generic generation (Z-Image-Turbo).", keyLabel: "MODELSCOPE_API_KEY", defaultUrl: "https://api-inference.modelscope.cn" }
  ];
  ...
  async function save(): Promise<void> {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.ok) throw new Error(data.error || "Failed to save image settings");
  ...
  async function runTest(): Promise<void> {
    ...
    try {
      const res = await fetch("/api/settings/image-generate/test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: testPrompt, engine: testEngine, imageGenerate })
      });
      const data = await res.json();
...
<div class="mx-auto flex max-w-4xl flex-col gap-6 px-6 py-8 sm:px-10 sm:py-10">
  <header class="flex flex-col gap-3">
    ...
    <div class="max-w-3xl space-y-2">
      <h1 class="text-3xl font-semibold tracking-tight text-foreground">Image Generation</h1>
      <p class="text-sm leading-6 text-muted-foreground">
        Configure the built-in Agent image generation tool. Multi-engine routing is supported across Agnes Image, Google Imagen, Volcengine, and ModelScope.
      </p>
    </div>
  </header>
    ...
    <form class="space-y-5" onsubmit={(event) => { event.preventDefault(); save(); }}>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select the default behavior for image generation commands.</CardDescription>
        </CardHeader>
        <CardContent class="grid gap-5">
          <div class="flex items-center justify-between gap-4 rounded-lg border bg-muted/30 px-4 py-3">
            <div>
              <Label for="image-enabled">Enable built-in imageGenerate tool</Label>
              <p class="mt-1 text-xs text-muted-foreground">When disabled, the tool returns a settings error instead of executing.</p>
            </div>
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div class="grid gap-1.5">
              ...
              <p class="text-xs leading-5 text-muted-foreground">
                In auto mode, the tool iterates through Agnes, Google, Volcengine, and ModelScope in order, using the first one with a valid API key configured.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-sm">Image Generation Engines</CardTitle>
          <CardDescription>Configure credentials and API endpoints for your selected painting providers.</CardDescription>
        </CardHeader>
            ...
            <div class="grid gap-3 rounded-lg border bg-muted/30 p-4">
              <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex flex-col items-end gap-1">
                  <Label class="text-xs text-muted-foreground">Engine enabled</Label>
                  <Switch bind:checked={imageGenerate.engines[engine.id].enabled} aria-label={`Enable ${engine.name}`} />
                </div>
              </div>
              ...
              <div class="grid gap-3 sm:grid-cols-2">
                ...
                <div class="grid gap-1.5">
                  ...
                  <p class="text-xs leading-5 text-muted-foreground">
                    Resolved URL: <code class="break-all font-semibold text-primary">{resolveCompleteUrl(engine.id, imageGenerate.engines[engine.id].baseUrl ?? "", imageGenerate.engines[engine.id].apiKey)}</code>
                  </p>
                </div>
              </div>
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-sm">Test Image Generation</CardTitle>
          <CardDescription>Test the configured settings in real-time. Unsaved values from the form will be used to attempt generating a test image.</CardDescription>
        </CardHeader>
        <CardContent class="space-y-4">
          <div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_220px_auto]">
            <Input bind:value={testPrompt} placeholder="Enter a prompt to draw" />
            ...
            <Button type="button" variant="secondary" onclick={runTest} disabled={testing}>{testing ? "Generating..." : "Test"}</Button>
          </div>
          {#if testResult}
            <div class="rounded-lg border bg-muted/30 p-4 text-sm">
              <p class="text-xs font-semibold text-foreground">ImageGenerateToolResponse</p>
