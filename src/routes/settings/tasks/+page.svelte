  ...
  <div class="grid h-full grid-cols-1 lg:grid-cols-[260px_1fr]">
    ...
    <section class="min-h-0 overflow-y-auto px-4 py-6 sm:px-8">
      <div class="mx-auto max-w-7xl space-y-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p class="text-sm text-slate-400">
              Inspect scheduled event tasks grouped by type and current
              execution state.
            </p>
          </div>
          <button
        ...
        {:else}
          <section
            class="grid gap-3 rounded-xl border border-white/15 bg-[#2b2b2b] p-4 text-sm text-slate-300 sm:grid-cols-2 xl:grid-cols-4"
          >
            <div>
          ...
          {#each typeOrder as type}
            <section class="space-y-3">
              ...
              {:else}
                <div
                  class="overflow-x-auto rounded-xl border border-white/15 bg-[#2b2b2b]"
                >
                  <table class="min-w-full text-left text-sm">
                    <thead
                      class="border-b border-white/10 bg-white/5 text-slate-300"
                    >
                      <tr>
                    ...
                    <tbody>
                      {#each rowsByType(type) as item (item.filePath)}
                        ...
                        >
                          ...
                          <td class="px-3 py-3">
                            <div
                              class={`inline-flex rounded-full border px-2 py-0.5 text-xs ${statusClass(item.status)}`}
                            >
                              {item.status}
                            </div>
                            ...
                            {/if}
                            {#if item.lastError}
                              <div class="mt-1 max-w-xs text-xs text-rose-300">
                                {item.lastError}
                              </div>
                            {/if}
                          </td>
