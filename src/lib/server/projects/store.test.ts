...
test("ProjectStore creates stable slugs, rejects duplicate roots, updates, and removes", () => {
  ...
  try {
    ...
    const updated = store.update(first.id, { name: "Renamed", instructions: "Keep it small.", modelKey: "custom|p1|m1", thinkingLevel: "max", toolProgress: "new", showReasoning: "off", runLogNotice: false });
