...
test("Agent Studio projects real activity into an accessible Three.js city", () => {
  ...
  assert.match(agentCityFallback, /onclick=\{\(\) => onSelect\(floor\.key\)\}/);
  assert.match(agentCityInspector, /tab === "overview"/);
  assert.match(agentCityInspector, /tab === "workers"/);
  assert.match(agentCityInspector, /tab === "runtime"/);
  ...
  assert.match(view, /profiles\.find\(\(item\) => item\.agentId === agentId\)/);
  assert.match(view, /chatStore\.newConversationDraft\(botId\)/);
...
test("Agent City owns WebGL lifecycle, quality fallback, and GPU cleanup", () => {
  ...
  assert.match(agentCityCanvas, /controller\.setVisualTheme\(visualTheme\)/);
  assert.match(agentCityScene, /agentCityRecipeProfile\(visualTheme\.recipe\)/);
  assert.match(agentCityScene, /floorPalette\(variant, dark, visualTheme\)/);
  assert.match(agentCityScene, /createWorkstation\(accent, visualTheme\)/);
  assert.match(agentCityScene, /createWorkerStation\(accent, subagent\.name, visualTheme\)/);
  assert.match(agentCityScene, /createRoomDecor\([\s\S]*visualTheme\)/);
  assert.match(agentCityScene, /visualTheme\.recipe === "technical"/);
  assert.match(agentCityScene, /new THREE\.GridHelper/);
  assert.match(agentCityScene, /visualTheme\.recipe === "retro" \|\| visualTheme\.recipe === "editorial"/);
  assert.match(agentCityScene, /visualTheme\.recipe === "expressive"/);
  assert.match(agentCityScene, /function applyMomoTheme/);
  assert.match(agentCityScene, /name\.includes\("momovest"\)/);
  assert.match(agentCityScene, /const steadyCloseUp = detailed \|\| focusedKey === node\.key/);
  assert.match(agentCityScene, /const stateChanged = previous !== floor\.state/);
  assert.match(agentCityScene, /if \(stateChanged\) node\.deskScreen\.emissiveIntensity/);
  assert.match(agentCityScene, /if \(stateChanged\) applyWindowGlow\(node, floor\.state\)/);
  assert.match(agentCityScene, /workerStatusChanged \|\| stateChanged/);
  assert.match(agentCityScene, /steadyCloseUp\s*\?\s*node\.windowBase/);
  assert.doesNotMatch(agentCityScene, /windowFlicker = night \? 0\.22/);
  assert.match(agentCityCommunityAssets, /visualTheme\?: AgentCityVisualTheme/);
  assert.match(agentCityCommunityAssets, /agentCityRecipeProfile\(visualTheme\.recipe\)/);
  assert.match(agentCityTheme, /"technical"/);
  assert.match(agentCityTheme, /"expressive"/);
  assert.match(agentCityTheme, /"retro"/);
