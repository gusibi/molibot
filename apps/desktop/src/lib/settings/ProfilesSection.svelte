...
{#if profilesStore.profileEdit}
  ...
  <form id="desktop-profile-form" class="entity-editor-form" aria-label={session.text.profiles} onsubmit={(event) => { event.preventDefault(); void saveProfileEditor(); }}>
    ...
    <div class="settings-form">
      ...
      <label class="settings-field"><span>{session.text.channelPermissionMode}</span><SelectControl value={profilesStore.profileEdit.permissionMode ?? "inherit"} ariaLabel={session.text.channelPermissionMode} options={[{ value: "inherit", label: session.text.permissionModeInherit }, { value: "plan", label: session.text.permissionModePlan }, { value: "manual", label: session.text.permissionModeManual }, { value: "accept_edits", label: session.text.permissionModeAcceptEdits }, { value: "auto", label: session.text.permissionModeAuto }]} onChange={(value) => updateProfileEdit((draft) => ({ ...draft, permissionMode: value === "inherit" ? null : value as "plan" | "manual" | "accept_edits" | "auto" }))} /></label>
    </div>
