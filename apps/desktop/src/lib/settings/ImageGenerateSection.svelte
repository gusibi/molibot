...
<p class="settings-section-hint">{session.text.imageGenerateHint}</p>
...
{:else}
  <div class="settings-card"><div class="settings-row"><strong>{session.text.webSearchEnabled}</strong><button class:active={toolsStore.imageGenerateEdit.enabled} class="switch" type="button" role="switch" aria-checked={toolsStore.imageGenerateEdit.enabled} aria-label={session.text.imageGenerate} onclick={() => { if (toolsStore.imageGenerateEdit) toolsStore.imageGenerateEdit = { ...toolsStore.imageGenerateEdit, enabled: !toolsStore.imageGenerateEdit.enabled }; markToolSettingsDirty("imageGenerate"); }}><span></span></button></div><div class="settings-row"><strong>{session.text.webSearchDefaultEngine}</strong><select bind:value={toolsStore.imageGenerateEdit.defaultEngine} onchange={() => markToolSettingsDirty("imageGenerate")}><option value="auto">{session.text.mediaEngineAuto}</option>{#each toolsStore.imageGenerateEdit.engines as engine (engine.id)}<option value={engine.id}>{mediaEngineLabel("image", engine.id)}</option>{/each}</select></div></div>
