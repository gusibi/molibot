...
{:else}
  <div class="settings-card"><div class="settings-row"><strong>{session.text.webSearchEnabled}</strong><IosSwitch checked={toolsStore.imageGenerateEdit.enabled} ariaLabel={session.text.imageGenerate} onCheckedChange={(checked) => { if (toolsStore.imageGenerateEdit) toolsStore.imageGenerateEdit = { ...toolsStore.imageGenerateEdit, enabled: checked }; markToolSettingsDirty("imageGenerate"); }} /></div><div class="settings-row"><strong>{session.text.webSearchDefaultEngine}</strong><SelectControl value={toolsStore.imageGenerateEdit.defaultEngine} ariaLabel={session.text.webSearchDefaultEngine} options={[{ value: "auto", label: session.text.mediaEngineAuto }, ...toolsStore.imageGenerateEdit.engines.map((engine) => ({ value: engine.id, label: mediaEngineLabel("image", engine.id) }))]} onChange={(value) => { toolsStore.imageGenerateEdit!.defaultEngine = value; markToolSettingsDirty("imageGenerate"); }} /></div></div>
  ...
  <p class="settings-group-title">{session.text.toolTest}</p><div class="settings-card tool-test-card"><div class="settings-form tool-test-form"><label class="settings-field"><span>{session.text.webSearchDefaultEngine}</span><SelectControl value={toolsStore.imageTestEngine} ariaLabel={session.text.webSearchDefaultEngine} options={[{ value: "auto", label: session.text.mediaEngineAuto }, ...toolsStore.imageGenerateEdit.engines.map((engine) => ({ value: engine.id, label: mediaEngineLabel("image", engine.id) }))]} onChange={(value) => toolsStore.imageTestEngine = value} /></label><label class="settings-field"><span>{session.text.toolImageSize}</span><SelectControl value={toolsStore.imageTestSize} ariaLabel={session.text.toolImageSize} options={[{ value: "1024x1024", label: "1024 × 1024" }, { value: "1536x1024", label: "1536 × 1024" }, { value: "1024x1536", label: "1024 × 1536" }]} onChange={(value) => toolsStore.imageTestSize = value} /></label><label class="settings-field settings-field-wide"><span>{session.text.toolPrompt}</span><input bind:value={toolsStore.imageTestPrompt} /></label></div><div class="settings-row-actions tool-test-actions"><button class="secondary-button" type="button" disabled={toolsStore.testBusy} onclick={() => void testToolSettings("imageGenerate")}>{toolsStore.testBusy ? session.text.loading : session.text.toolTest}</button></div>{#if toolsStore.testResult}<pre class:run-history-failed={!toolsStore.testResult.ok} class="tool-test-result">{JSON.stringify(toolsStore.testResult.result ?? toolsStore.testResult.error, null, 2)}</pre>{/if}</div>
