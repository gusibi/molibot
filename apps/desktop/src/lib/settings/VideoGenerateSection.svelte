...
{:else}
  <div class="settings-card"><div class="settings-row"><strong>{session.text.webSearchEnabled}</strong><IosSwitch checked={toolsStore.videoGenerateEdit.enabled} ariaLabel={session.text.videoGenerate} onCheckedChange={(checked) => { if (toolsStore.videoGenerateEdit) toolsStore.videoGenerateEdit = { ...toolsStore.videoGenerateEdit, enabled: checked }; markToolSettingsDirty("videoGenerate"); }} /></div><div class="settings-row"><strong>{session.text.webSearchDefaultEngine}</strong><select bind:value={toolsStore.videoGenerateEdit.defaultEngine} onchange={() => markToolSettingsDirty("videoGenerate")}><option value="auto">{session.text.mediaEngineAuto}</option>{#each toolsStore.videoGenerateEdit.engines as engine (engine.id)}<option value={engine.id}>{mediaEngineLabel("video", engine.id)}</option>{/each}</select></div></div>
