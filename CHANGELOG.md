...
- **错误噪音抑制**: Weixin channel 现在会吞掉本轮中间工具进度和工具/模型错误详情；如果本轮最终没有正常答案，才发送最后缓存的一条错误说明。
- **QQ 错误噪音抑制**: QQ channel 现在使用同样的 channel-local 缓冲策略，成功运行不发送中间错误；整轮没有正常答案时只发送最后一条错误说明。
