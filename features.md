...
| ENG-102 | Externalized Telegram runner instruction files | Done | `runner.ts` now loads system prompt from workspace `AGENTS.md` at runtime (plus optional `SOUL.md`/`TOOLS.md`/`BOOTSTRAP.md`/`IDENTITY.md`/`USER.md`), renders runtime placeholders, and falls back to bundled default template when workspace file is absent |
