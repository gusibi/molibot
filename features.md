...
| ENG-97 | Pluggable memory gateway + core + plugin switch | Done | Added stable memory gateway API with replaceable core (`json-file`), `add/search/flush/delete/update` API endpoints, chat-time memory injection, and `/settings/plugins` memory enable toggle |
| ENG-98 | Memory v2 layered + incremental retrieval pipeline | Done | Added layered memory (`long_term`/`daily`), core capability negotiation, incremental `flush` cursor, hybrid search (keyword+recency), and per-scope markdown mirrors (`MEMORY.md` + `daily/*.md`) |
...
- 2026-02-25: Added pluggable memory architecture (`memory gateway` + replaceable `memory core`) with built-in JSON-file core, `POST /api/memory` operations (`add/search/flush/delete/update`), prompt-time memory retrieval injection, and new `/settings/plugins` memory enable/core settings UI.
