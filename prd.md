...
- **P1-B (a) — Delivered (2026-07-26)**: Project Chat forks instead of truncating. Project-scoped fork authorization is `SessionStore.getForkableConversation` (Web stays owner-gated; Project is owner-shared, matching how project conversations are already continued by id); child selection reuses `selectProjectSession`. The destructive `DELETE /api/sessions/[id]/messages` endpoint now has no Desktop caller.
- **Deferred P1-B (b)**: In-Session leaf navigation and generated branch summaries remain a separate slice. Lineage is stored (`parentSessionId`, `forkedFromMessageId`) and the sidebar marks a fork with `↳`, but there is no sibling/child navigator inside a Session and no generated summary of what a branch diverged on. Blocking question to answer first: whether branches are presented as a tree in the sidebar or as an in-transcript leaf switcher at the fork point — the storage supports either.
