...
- 小程序由用户单独实现；本次不增加小程序入口、宿主桥或小程序包。后续复用同一份报告和 HTML 渲染能力。
- 验收：消息准确定位，父子节点与并行耗时正确，token 不重复，未知用量可识别，公开报告去摘要，插件开关可重启保存，APP 冷启动和服务恢复可查看。
- 使用说明：[调用链报告](docs/guides/trace/call-trace.md)。

# Molibot PRD (V1)

## 计划管理、面板编辑与跨 Session 续跑（2026-09-13）

- **优先级 / 阶段**：优先级待定 / 方案 v4；共享计划服务、桌面「计划」看板、生成即保存与首次批准接入 Durable Execution 已落地（2026-09-13，见 `features.md`）；跨 Session 显式认领、看板内确认/审批、首次批准权限选择、实时事件未交付。完整需求见 [计划看板 PRD](docs/requirements/plan-board-prd.md)。
- **第一版范围**：稳定计划 ID、统一列表、右侧两层编辑、真实进度与断线恢复、暂停修订、跨 Session 继续同一计划；推荐复用 Durable Execution。已按此决策把计划存为 Durable Execution 聚合并新增两层任务表。
- **验收重点**：A 会话执行部分工作后，B 会话可从列表找到并继续同一计划；保留执行上下文与已完成结果，不重复执行、同计划不由两个 Session 同时执行、不丢编辑草稿；不同任务独立执行，目标 Session 忙碌不阻止继续，允许抢占调度。
- **设计边界**：拟扩展原 Session 计划执行约定；实施前同步 DESIGN，当前交付状态不因本方案改变。批量大计划与多列看板留待后续阶段。

## Session 累计用量与上下文容量（2026-09-12）

- **阶段**：已实现并通过共享集成验收（2026-09-12）；完整范围与验收见 [Session 用量需求](docs/requirements/session-usage-summary.md)。
- **已确定行为**：回复下方保留本轮用量；输入框统计面板以整个 Session 累计用量为主，当前已使用上下文（最后一次输入 + 该次回复输出）另列。压缩上下文或删除部分消息不能扣减已经发生的用量。
- **验收重点**：服务端按 Session 汇总共享用量记录；缓存命中比例按累计 tokens 计算；普通 Chat 与 Project Chat 一致，切换会话和重启不串数、不丢数。

## 统一执行权限模式与沙箱后端入口（2026-09-11，issue #49）

- **阶段**：已实现并通过共享集成验收（2026-09-11）；完整范围与验收见 [issue #49](https://github.com/gusibi/molibot/issues/49)。
- **决策**：设置默认值与对话窗口共用计划、手动、接受修改、全自动四种模式；删除独立沙箱四档及启用覆盖。全自动直接使用宿主环境，所有可用工具遵循共享免审批策略；保留身份与所有权校验、进程隔离、取消和超时。
- **执行环境**：保留当前 Anthropic 本地沙箱、诊断与高级限制；域名限制默认不限制，仅影响沙箱命令，全自动下不适用。共享执行入口（`agent/exec/executionBackend.ts`）与工作区身份已落地，后端替换无需改动会话/Channel 逻辑；本次未接入第二家后端。
- **验收**：`approvalSuspension.test.ts` 已扩展为真实 Runner/Agent/工具调度/审批存储的主验收入口，覆盖全自动宿主执行、显式宿主请求、denyWrite 文件、管理操作、渠道 Manual 不放宽、模式切换时机与待审批不被复活；设置 round-trip、MCP third_party 调度、外部适配器翻译均有专项测试。
- **取代的旧决策**：下文权限模式与沙箱预设条目中“两套独立四档”“Auto 不提供完全访问”“安装在所有模式下必须审批”的旧口径已由本实现取代，对应测试期望已替换。
...
## 审批续跑展示与链路审查（2026-09-06，2026-09-10 更新）

- **P0 / 已实现，原生验收待验证**：后台统一记录一次最终回答及结构化活动；Desktop 按服务端运行状态等待，耗时不包含之前的审批等待。
- **P1 / 已交付（2026-09-10，issue #48）**：统一 Host Bash 内联等待器与异步审批处理器的执行认领和结果交付——Web/Desktop 出带执行入口已使用原子 `claimExecution`，认领失败按记录真实状态答复；挂起、决定与恢复的共享语义由 `approvalSuspension.test.ts` 以真实 Runner/Agent/Broker 全链路回归锁定。
- **P1 / 部分交付（2026-09-10，issue #48）**：通用工具审批恢复已统一按审批 ID（`approvalRequestId`）定位等待结果，Runner 挂起状态强制关联持久化请求；「恢复携带项目与会话模型配置」已由既有 `resumeWithToolResult`/`buildRunnerProjectContext` 链路承担，运行控制文本清理（挂起结果改写为规范 Runtime Notice）已覆盖主链路，遗留的模糊匹配清理项并入后续运行链路审查。
- **P2 / 待评估，未排期**：为无 SSE 的续跑提供可重连的事件订阅和明确的排队/执行/完成回执，取代 Desktop 轮询；服务重启后仍能确认原审批的执行结果和续跑归属。重启后的结果核对已交付崩溃恢复核对（stale executing → 结果未知终态，不盲重跑）。

## 审批等待成为明确暂停状态（2026-09-10，issue #48）

- **P0 / 已交付**：进入审批等待后任务停止新模型轮次与后续工具（含同批次未启动动作与 steer 注入）；等待状态强制关联持久化审批请求；run 行落 `waiting_for_approval`；拒绝/过期/取消为明确终态；Broker 待审批读路径按共享 TTL 过期；Stop 中等待 → 请求过期；Desktop 服务端确认前保留卡片、提交中防重复、失败可重试、多请求稳定顺序、页面恢复以服务端为准。
- **验收**：`approvalSuspension.test.ts`（真实 Runner + Agent 循环 + Broker @ 临时 DATA_DIR）覆盖「发起任务 → 等待审批 → 提交决定 → 继续执行」全链路；Desktop 控制器与结构守卫覆盖卡片保留/防重复/恢复/顺序。
...
## App 会话列表轻量查询（2026-09-19，已交付）

- **优先级**：未单独排序；本次确认实现范围。
- **范围与验收**：App 的 Web、外部渠道、Project、会话管理普通列表及分页只读 UI Session 元数据，正文读取为零；保留过滤、排序、分页与搜索弹窗预览，验证实际延迟下降。
- **交付**：已实现并通过隔离数据实测与冷启动冒烟；结果、未完成检查与文档影响见规范实施记录。
- **规范**：[App 会话列表的轻量查询](docs/requirements/app-session-list-metadata-spec.md)。
