...
- [Done] `webSearch` 放进 deferred registry 后，必须保持 deferred-only，不再作为顶层 lightweight stub 一起暴露；否则 `toolSearch` 加载后会把同名工具重复带进 provider `tools` 参数并触发 400。
