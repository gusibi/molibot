...
| P1-183 | Agent bash OS sandbox boundary | P1 | Delivered (2026-05-10) | Operators can opt into OS-level sandboxing for main and built-in subagent `bash` only, configure env/network/filesystem policy in Settings, inject allowlisted workspace env-file keys without leaking host process env wholesale, inspect redacted diagnostics, and see `bash (sandbox)` in tool output when the sandbox path is used; host-browser, desktop app, ACP, MCP, and channel transport access remain outside this sandbox |
