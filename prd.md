...
- Subagent delegated runs need their own operator-controlled text model routing so expensive main models can delegate to cheaper/faster models without changing the main conversation route. Built-in subagent roles should reference abstract model levels (`haiku`, `sonnet`, `opus`, `thinking`) that operators map to configured models, rather than showing concrete Claude model IDs that may not exist locally. Built-in roles should be visible as read-only runtime inventory, and channel/client run traces should make subagent usage visible without dumping long tool outputs.
