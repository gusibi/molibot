...
- **System Prompt Boundary Refactor**: Simplifies and modularizes the static system prompt by removing duplicated event scheduler details, tool schemas, and low-level sandbox OS implementation details (such as `sandbox-exec` and `bubblewrap`). Simplifies the prompt to contain only high-level decision routing and policy boundaries while aligning the `bash` tool definition schema. Includes dedicated regression assertions to protect prompt caching stability.
