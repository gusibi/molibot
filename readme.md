...
- **Skill Usage Trace Facts**: Trace analytics now records implicit skill loads when the model successfully reads a loaded skill's `SKILL.md`, and records successful `skillSearch` matches as informational triggered candidates. `skill_usage` facts preserve monotonic confidence via `payload.level` plus `payload.evidenceCsv`, so search-only signals do not imply loaded/executed usage. Implementation progress for the broader skill tracking plan is tracked in `docs/trace/skill-usage-tracking-progress.md`.
