...
- **Friendly recurring schedules**: Desktop Automations creates daily, weekly multi-select, monthly-by-date, or custom Cron plans while keeping watched-event JSON as the runtime source of truth and preserving existing complex Cron expressions during edits. New tasks select an enabled Bot and one of its configured allowed Chat IDs; internal storage directories and recipient-less workspaces are not exposed as targets.
...
- **Scheduled Tasks**: watched event JSON remains the scheduling source, while active event execution is coordinated through `event_execution_leases` in SQLite for timeout, retry, restart/stop visibility, and run correlation. Desktop Automations provides recurring-task creation and maintenance, compact three-run summaries, manual execution, and a dedicated on-demand SQLite-backed history modal with pagination.
