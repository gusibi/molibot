...
- **Telegram Overlong Edit Resilience**: Telegram editable status/answer/detail messages now share one chunked text-delivery path. If `editMessageText` hits `MESSAGE_TOO_LONG`, Molibot keeps the first chunk in the edited message and sends the rest as follow-up messages instead of aborting the run.
...
- **Built-In Video Generation**: `videoGenerate` is a shared Agent-layer deferred tool supporting Agnes Video and Volcengine (Doubao Seedance) engines. It maps credentials from `AGNES_API_KEY` and `VOLCENGINE_API_KEY`. Users can customize the exact model ID for each provider (defaulting to `agnes-video-v2.0` and `doubao-seedance-2.0` respectively) via the settings UI. The Agent routes video generation intent in any language through `toolSearch select:videoGenerate`. The tool performs path validation, saves generated videos locally, and uploads them back to the channel. Settings are fully manageable via the `/settings/video` page, which supports provider credentials, enable toggles, custom model selection, custom base URLs, and live video generation connectivity tests.
