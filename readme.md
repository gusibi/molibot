...
- **Skill Usage Trace Facts**: Trace analytics now records implicit skill loads when the model successfully reads a loaded skill's `SKILL.md`, and `skill_usage` facts preserve monotonic confidence via `payload.level` plus `payload.evidenceCsv`. Implementation progress for the broader skill tracking plan is tracked in `docs/trace/skill-usage-tracking-progress.md`.
