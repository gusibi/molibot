...
- **Reliable Bash Stop Finalization**: `/stop` terminates an aborted bash/Agent run without empty-response retries or model fallback, drops pending progress updates, sends a terminal stopped confirmation, and returns the session to idle after the command process is killed.
