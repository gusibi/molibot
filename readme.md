...
- **Add image engines without code changes.** Image Generation settings can register multiple custom endpoints using either `images/generations` or `chat/completions`, route them through the Agent's generic provider, keep the chosen protocol immutable, and persist the configuration across restarts.
