...
- **Fresh automation archives without context carry-over**: Each recurring task with a stable task ID reuses one hidden archive Session instead of creating a Session per trigger. Every execution still starts from empty model messages, persists transcript entries under its own `runId`, opens only that run from execution history, and restores the prior active chat Session afterward. Legacy tasks without stable IDs and historical per-run Sessions remain compatible.
...
- **Native Desktop behavior layer**: The macOS Desktop app shares one localized command catalog across App Menu, Tray, and ⌘K. Empty ⌘K searches rank successful local recent commands, active-workspace actions, and catalog recommendations, while typed searches retain text relevance first. Its versioned local history stores only stable command IDs, successful-run counts, and local timestamps (20 items, 90 days)—never query text, user content, labels, parameters, session/profile identifiers, or errors. It also adds recoverable startup, shared accessible dialogs, persisted close behavior, direct manipulation, focused native window chrome, visibility-aware activity scheduling, and privacy-safe foreground/background feedback. Local Apple Silicon packaging produces a checksum-verified DMG; Finder layout timeouts fall back to a functional installer without custom icon placement. An independently compiled, isolated QA bundle has also verified standard native bundle launch, resource resolution, managed-sidecar startup, and single-instance behavior without disturbing an existing debug host.
...
- **Clipboard image attachments with live recognition**: Desktop Chat and Project Chat accept screenshots pasted directly from the clipboard as pending image attachments, collapsing multiple clipboard formats for one screenshot into a single file. Image turns stay on multipart SSE and visibly progress through upload, image recognition, and token-by-token Assistant output instead of waiting for one final refresh.
- **Governed daily-material pipeline**: An internal watched event turns explicitly authorized conversation projections into a redacted, dated Markdown file inside a registered Project. It uses an independent watermark and Project-owned prompt template, never enters ordinary chat history, and fails closed on invalid paths, credentials, aborts, or disconnected Projects. Completion is aggregated once across Bot scopes and sent to the same authorized Telegram/Feishu destination used by memory reflection.
...
- **Resilient governed memory**: Daily reflection runs at a Desktop-configurable local time, processes the previous complete local day, and shares one selected authorized Telegram/Feishu notification destination with daily materials. Internal completion notices use a context-free channel path: they do not run an Agent, append Agent history, or replace the active Session. Reflection still isolates malformed LLM candidates while preserving infrastructure retries, detects embedding credential rotation without exposing secrets, temporarily falls back to lexical retrieval after provider failures, and keeps 10k-record compaction membership linear-time.
...
- **Desktop Workspace Surface Hierarchy**: Desktop Chat keeps navigation and file panes on the sidebar surface while the Header and transcript form one continuous primary workspace. Desktop Settings keeps its navigation stable while the right-side Header/content use a secondary canvas with primary-surface cards and quieter borders. Both use DESIGN-derived tokens across light and dark themes.
- **Consistent macOS Settings Controls**: General, Skills, Search, Image, Video, Voice, Host Bash, Web Profile, Sandbox, and Plugins share one accessible `IosSwitch` implementation, so checked, disabled, focus, light/dark, bilingual, and compact-window states remain visually and behaviorally consistent.
...
- **Readable and bounded Agent Sessions**: IM `/new` sessions use date-scoped IDs with a four-letter random suffix such as `s-20260622-yush`. A fresh recurring task with a stable task ID now reuses one hidden per-task archive Session; every execution still starts with empty model context and its history remains selectable by run. Legacy tasks without a stable ID retain the date-scoped `task-20260622-yush` form.
