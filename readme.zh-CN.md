...
- **在无视觉干扰下保持聊天上下文可见**：垂直对齐的 `# source / title` 头部可以轻松区分 Web、飞书、Telegram、QQ、微信和项目会话；其整个被动区域都支持拖拽本地窗口，安全缩进且与设置页共用 `228px` 默认宽度的紧凑侧边栏让常用目的地触手可及，上下文时间戳会在消息非今天发出时加上昨天或日期。
