...
- **在 macOS 受保护目录中继续使用 Project**：当 iCloud、桌面或文稿中的 Project 被系统拒绝读取时，右侧文件面板提供原生的同目录重新授权入口，不会只留下原始文件系统错误，也不会静默替换 Project 根目录。
- **直接预览 Project 会话生成的图片**：即使对话属于 Project，写入本轮 Session 临时空间的图片仍保留自己的文件作用域，可从“本轮文件”进入右侧图片预览。
...
- **一眼识别外部渠道**：桌面端 Chat 左侧 Session 列表为 Telegram、QQ、微信和飞书使用随包内置的 Reicon 品牌 SVG（飞书使用 Doubao Logo），保持现有 16px 图标槽位；没有品牌资源的渠道继续使用本地中性回退图标。
- **让 Web 也有明确的视觉锚点**：桌面端 Chat 左侧 Session 列表为 Web 使用随包内置、纯代码生成的 Safari 风格彩色指南针 SVG，以蓝青渐变和红白指针保持同一 16px 图标槽位，不依赖网络。
