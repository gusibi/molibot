...
2. HookManager
3. SkillRegistry
4. ContextBuilder.addSkill()
5. TraceRecorderHook
6. agent_turns 表
7. agent_skill_usages 表
8. agent_model_calls 表
```
...
## Step 2：实现 HookManager

先支持内置 hook，不急着支持 script hook。

```ts
hooks.register("skill.injected", new SkillUsageHook())
hooks.register("model.call.after", new TokenUsageHook())
```
...
把所有 `read SKILL.md` 收口到这里。

---

## Step 4：实现 ContextBuilder.addSkill

禁止手动拼 skill 字符串。

---

## Step 5：记录 skill.selected / loaded / injected

先把 skill 的生命周期记录完整。

---

## Step 6：实现 model usage 记录

从模型 API usage 字段记录 token。

---

## Step 7：实现 subagent run 继承 turn_id

让 subagent 也进入同一条 trace。

---

## Step 8：再开放 script hook
