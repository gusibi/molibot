...
1. lease acquisition only allows one active attempt per event/session
2. timeout watchdog marks timeout and triggers abort
3. attempt counter stops at 3
4. manual stop suppresses retry
5. restart cleanup releases stale running lease
6. shared busy detection blocks fresh prompt while event lease is active

### Integration Tests

1. periodic event starts, hangs, times out, retries, then succeeds
2. periodic event hangs three times and ends failed
3. `/stop` during scheduled run returns stop confirmation and no retry
4. user message during active scheduled attempt is queued instead of causing concurrent prompt error

### Manual Verification

1. create a periodic event with a deliberately hanging `bash`/network command
2. confirm visible timeout after configured threshold
3. confirm old run is aborted before retry begins
4. confirm at most 3 attempts
5. confirm final status visible in event file and runtime detail
6. confirm `/stop` no longer returns `Nothing running.` during the active stuck attempt

## Migration Plan

1. add lease store and schema
2. wire event watcher entrypoints to acquire leases
3. add timeout watchdog and abort integration
4. update `/stop` shared semantics
5. update startup cleanup to reconcile stale leases and running turns
6. add config plumbing
7. add tool timeout hardening for known hanging external command paths
8. add tests
