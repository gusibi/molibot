...
- 状态：**已完成（2026-07-17）**。v3.2 的 C0/C1 与 T10–T18 已交付；权威契约、边界和六组顶层验收保留在 [`docs/archive/requirements/memory-improvement-plan-v3.md`](docs/archive/requirements/memory-improvement-plan-v3.md)。
...
- [Spec] 完整产品、数据、API、生命周期、测试和验收契约见 [`docs/archive/requirements/memory-trace-and-memory-center-prd.md`](docs/archive/requirements/memory-trace-and-memory-center-prd.md)。
...
- [Planned, P2] 后续按 `docs/archive/requirements/agent-workbench-plan.md` 接入 Sub-agent 与更细粒度的任务派发事件动画；共享运行状态仍归 Agent/app 上层，不下沉 Channel。
...
- 详细契约、数据模型、接口、安全边界、实施切片与验收矩阵见 `docs/archive/requirements/project-session-provenance-and-inspection.md`。
...
- 背景：定位收敛为「记忆优先、可审计、长期陪伴的个人 Agent」（魔魔计划）；审计发现 mory SDK 能力足够但宿主接线不足。经三轮外部 review 修订至 v2.2，详细契约、任务书与端到端验收见 `docs/archive/requirements/memory-improvement-plan.md`。
...
- 拆出：T8 Subagent 信息采集 → `docs/archive/requirements/content-collection-pipeline.md`；T9 Project 级技能加载 → `docs/archive/requirements/project-skills-loading.md`（记忆 MVP 不依赖两者）。
...
- 详细实施方案（分 5 个 Slice，含代码锚点、每步理由与验证清单）：`docs/archive/requirements/projects-feature-plan.md`。
...
- `docs/guides/plugins/authoring.md (moved from `docs/plugins/authoring.md`; see `docs/archive/requirements/` for the superseded `plugin-development.md`)`: plugin development contract, lifecycle, config shape, and current discovery/runtime boundaries.
