# Molibot Features Archive - 2026 Q3

## 2026-08-05

### 内置小程序一键更新（只覆盖代码，保留数据）（已完成，P2）

内置小程序升级后，已安装的旧版本没有升级路径：bootstrap 出于「不能覆盖用户改过的副本」的设计从不覆盖已存在目录，用户只能卸载重装，而那恰恰是最容易丢数据的操作。

- 目录现在会给出 `updateAvailable` / `availableVersion`：用 semver 比较，随包版本更低不会提示「降级」，版本相同不提示；任一侧无法解析（含加载失败时的 `unknown`）则视为可更新，因为重写随包副本正是修复损坏内置应用的手段。
- `MiniAppHost.updateBuiltin()` 沿用卸载的时序纪律：挂起 → 等待在途调用排空 → `dispose()` → 才动文件系统（应用可能正持有目录内的 SQLite 句柄）。代码目录整体替换，旧版多出的文件会消失；数据目录完全不动；启用状态保留（原本关闭的更新后仍关闭）。
- 落盘逻辑收敛到 `builtinPackage.ts` 一处，首次安装与更新共用：先写入点号前缀的暂存目录再原子改名。点号前缀很关键——发现逻辑跳过点号条目，所以写入过程中的暂存目录不会变成一条「损坏的应用」。
- 新增 `POST /api/desktop/miniapps/update`，与 `/install` 分开：安装接收的是用户选定、需要信任提示的来源，更新只接收 appId，写入的永远是用户当前正在运行的 Molibot 自带代码，因此不需要信任确认，也不询问数据。
- 设置 › 小程序在版本号旁显示「有新版本 v1.0.1」徽标，并只在该行显示「更新」按钮。与安装一致，新代码需重启服务后生效（被替换的模块已在 ESM 缓存中），提示文案复用同一条。

### 小程序列表只展示真正的小程序（已完成，P2）

用户反馈：小程序目录里的 zip、散落文件、无关文件夹都会出现在小程序页面，且既装不上也删不掉，纯属干扰。

- `MiniAppHost.refresh()` 现在只把「包含 `manifest.json` 文件的目录」当作候选条目；其余条目直接跳过，不再生成错误行。
- 声称是小程序的目录（有 manifest）仍保留全部既有错误提示：JSON 解析失败、id 不匹配、未知字段、引擎版本、工具 schema 非法、目录名不合法、符号链接目录等，坏安装依然可见。
- 回归测试 `non-app clutter in the code root never reaches the catalog` 覆盖 zip / 散落文件 / 临时目录 / 非法目录名 / 无 manifest 的目录。

### AI 服务商模型验证结果归位弹窗（已完成，P2）

模型编辑弹窗里的「验证连接」原本把成功/失败写入服务商页面的通用消息，因此提示显示在遮罩后的外层页面，和触发操作脱节。

- 模型验证改为返回弹窗局部结果，不再写外层 `actionMessage`。
- 成功和失败统一显示在弹窗底栏「验证连接」按钮左侧，并使用明确的本地化状态文字；长错误就地截断并通过 title 保留完整内容。
- 模型弹窗打开期间隐藏背景服务商页面遗留的通用提示；关闭后其它服务商级提示可以恢复，而模型验证结果永远不会进入该通道。
- 验证返回后同步更新当前模型草稿的角色与能力验证状态；请求期间若关闭弹窗或切换服务商，迟到结果会被丢弃。
- 结构测试锁定底栏状态分组，并禁止模型验证重新污染服务商页面的全局消息。

### 内置 Agent / 内置 Skill 支持安装后升级（已完成，P1）

用户反馈：「内置 Agent 和 Skill 一旦安装之后就没办法更新了」。

**问题**：内置 Agent 模板在应用包里，安装后的副本在 `<dataRoot>/agents`，两者之间没有任何关联——新版 Molibot 修好的模板只对「从没装过的人」生效。内置 Skill 本来就有「版本号变化 → 启动时自动升级」的机制，但界面上看不到装的是哪个版本，owner 自己改过的副本、或者删掉又想要回来的情况也没有入口。

**做法**：
- 内置 Agent 模板加版本号：写在模板 `AGENTS.md` frontmatter 的 `version:`（缺省 1.0.0），17 个模板都已显式写上，bump 点可见。
- 新增安装账本 `<dataRoot>/agents/.builtin-agents.json`：记录写入的版本 + 每个文件写入时的 sha256。前者用于判断「有更新」，后者用于分辨「原样副本」和「被改过的副本」。
- 设置 → Agents（桌面端 + Web）展示：随包版本、已装版本、「有更新」标记、「本地已修改」标记，以及「更新」按钮。**只手动触发**——Agent 提示词正是 owner 会改的东西，不做启动自动覆盖。
- 覆盖策略（按 owner 确认的方案）：被改过的副本先整目录改名成 `<id>.backup-<时间戳>` 再写入新版本，界面回显备份路径；owner 自己新增的文件保留在活动目录里。装在账本出现之前的副本无法自证来源，一律按「已修改」处理（备份），并且**故意**报告为可更新——这些副本必然比当前构建旧。
- settings 里的 Agent 行会跟随模板的名称/描述更新，但 owner 配的启用状态、模型路由、沙箱一律保留。
- 内置 Skill 同样暴露版本状态并新增手动应用：`GET /api/desktop/skills` 增加 `builtins[]`，`PUT` 增加 `{kind:"builtin", id}`。手动路径刻意跳过两道「为自动行为服务」的闸门——版本相等检查（因此能修复被改坏的同版本副本）和删除墓碑（明确请求就是要它回来），但保留备份保证。启动行为不变。
- 复用而非复制：哈希、分歧判断、staging+rename 交换、备份逻辑抽到 `src/lib/server/agent/bundles/materializedBundle.ts`，Skill bootstrap 与 Agent 模板成为同一套实现上的两种策略。
- 顺手修复：`.status-badge[data-state="warning"]` 在「跟随系统」外观（无 `data-theme` 属性）下没有规则，会用浅色 `--warning-text` 画在深色底上（pitfall 4），新的「有更新」标记正好踩中。

**守护**：`builtInAgentTemplates.test.ts` 5 个新用例、`skills/bootstrap.test.ts` 3 个新用例、`desktopSkills.test.ts` 1 个新用例。

**验证**：agent 模板 7/7、skill bootstrap 14/14、`test:desktop-chat` 240/241（唯一失败项在干净分支上同样失败）、桌面 `svelte-check` 0/0、桌面 UI 测试 141/141、两个 `vite build` 通过；并在临时 `DATA_DIR` 上起真实服务走完整流程：安装 → 改副本 → 发布新模板版本 → 更新 → 新内容生效、自加文件保留、改动在备份目录里可找回。

### 项目设置「自定义命令」编辑器重排（已完成，P2）

用户反馈：「这么多框也没有对齐，看着很乱」。

**问题**：一条命令由三个各自带边框的控件竖排组成，删除按钮挤在第一行里，导致命令名输入框比下面的说明/内容窄 28px，右边缘参差；而分组本身已经有一圈边框，字段再各画一圈，形成「框套框套框」。焦点态还叠了 `card-bg` + `accent` 双环，看起来像被框了两次。

**做法**：
- 每行改为 `max-content / 1fr / 28px` 网格并显式指派单元格：标签独占一列，三个字段共用同一左右边缘，删除按钮占固定的第三列「留白槽」，永远不会挤短上面的字段。（自动流排布在这里是错的，会把下一行标签填进故意留空的槽里。）
- 三个字段各自带标签（命令 / 说明 / 内容），不再只靠 placeholder 表意，同时保证每个控件都能被标签点中。
- 字段由「带边框的盒子」改为分组面上的「凹槽」（`--control-bg` on `--surface-secondary`），去掉第三层边框。项目设置内的输入框、文本域、下拉框、命令凹槽和按钮统一采用 AppKit 中性聚焦态（`--control-border-strong` + 8% `--label-primary` 柔光），不再沿用突兀的蓝色 Geist 焦点环。命令名内部输入框显式清除通用设置字段的聚焦阴影，由外层组合控件统一承担焦点提示。
- 空列表给出明确文案，而不是只剩一个「添加命令」按钮。
- 该区块字号统一走类型标度 token（`--fs-label` / `--lh-label` / `--icon-sm`），替换手写的 12px。

**守护**：`apps/desktop/src/chat-ui.test.mjs` 断言网格模板与留白槽、凹槽/分组配色角色、项目设置焦点规则不含 accent 蓝色、必须使用中性语义 token、嵌套命令名输入框必须重置自身聚焦阴影、三个标签与空态文案存在。

**验证**：桌面 UI 测试 141/141 通过，`svelte-check` 0 错 0 警，`vite build` 通过；Light / `data-theme="dark"` 两种外观 + 焦点态 + 空态均在真实渲染中确认。
...
- 新增 **Mini App** 机制：一个应用同时提供 Agent 工具、自带 UI 和一份私有数据，三者共用同一个领域模块，因此对话里加的待办和面板里看到的待办永远是同一份数据。安装后由 Host 立即激活；数据独立存放在 `miniapps/data/<app-id>/`，安装、升级、卸载都不会被动到（卸载时是否删除数据由用户明确选择）。
...
- **入口、安装与图标（对首版的补充）**：
  ...
  - 支持三种图形化安装：本地目录、ZIP 压缩包、GitHub 仓库（`owner/repo` 或 github.com 链接，可选分支/标签）。安装先落到临时暂存目录并在那里完成 manifest 校验，校验通过才原子移入安装根——因此坏的来源什么都装不上，覆盖安装失败时旧版本原封不动；安装完成后 Runtime 在当前服务内立即激活。
...
- 需求与实施方案：[miniapp-platform-prd.md](docs/archive/requirements/miniapp-platform-prd.md)、[miniapp-platform-implementation-plan.md](docs/archive/requirements/miniapp-platform-implementation-plan.md)。
...
- **窄窗口下面板按钮失效修复**：面板一旦改成 `position: fixed` 覆盖层就必须带 `z-index`，而该 `z-index` 会让面板成为层叠上下文，把面板头部（`z-index: 31`，本就是为了盖过拖动遮罩）困在 `z-index: 30` 的窗口拖动遮罩之下——关闭/刷新按钮点不动且控制台无任何报错；同时对话区 header 的操作行仍按全宽布局，会直接画穿面板头部。另有两个 media 层级同时重声明窄屏 `with-files` 网格，导致覆盖层旁边多出一条空轨道。现在窄屏拆分只由一个层级负责，面板始终在流内；侧栏隐藏后 header 自行让出交通灯区域（`padding-left: 84px`）。
- **拖拽手柄对齐**：两个 resizer 是按存储宽度绝对定位的，因此 `ChatView` 用同一套预算（`viewportWidth − 侧栏 − CHAT_MIN`）钳制存储值，并通过 `bind:innerWidth` 跟随窗口尺寸；用户偏好宽度保留，窗口变宽后自动恢复。
- **Header 重叠修复**：标题的 `max-width: 46vw`、消息/输入框的 `clamp(20px, 5vw, 56px)` 边距、模型菜单的 `38vw` 上限都是按窗口而非按栏计算的，面板压窄栏宽后仍保持全窗口数值，导致标题压到操作按钮下方、输入框控件溢出。全部改为按栏百分比；标题块 `flex: 1 1 auto` 吃掉剩余空间、操作区取自然宽度，标题改为省略号截断。
- **文件面板对齐 macOS 设计语言**（`DESIGN.md` macOS 产品层）：
  - 源列表选中态改为 AppKit 语义——列表未获焦点时用 `unemphasizedSelectedContentBackground` 灰，获得焦点时用 accent 填充且图标文字转白；键盘游标改为焦点环，不再与选中态争夺同一种填充。
  - 行高 24px、缩进 14px，文件名改用系统字体 13px（等宽字体按规范只保留给尺寸、路径与代码）。
  - 分段控件由 32px iOS 药丸改为 22px AppKit 规格（2px 内衬、7/5px 圆角、选中段抬升到控件面）；查看器标签页由 accent 下划线改为抬升选中页。
  - 面板头部去掉蓝色实心文件夹图标与实心圆形关闭按钮，统一为 28px 单色线性图标；底部状态条由 48px 控件行压缩为 28px 静默状态条。
  - **中文路径乱码修复**：diff 顶部路径显示为 `02-\345\206\205…` 八进制转义。`core.quotePath` 默认为 **true**，而项目巡检 runner 为保持无污染执行会丢弃 `HOME` 与系统配置，用户全局的 `quotepath=false` 根本不会生效，`git diff` 于是在 `diff --git` 与 `---`/`+++` 头部转义所有非 ASCII 路径；`git status --porcelain -z` 因为是 NUL 分隔的原始字节而不受影响，所以变更列表正常、diff 头部异常。现所有 git 调用统一加 `-c core.quotePath=false`，并在 `inspection.test.ts` 用真实中文路径仓库做回归（去掉修复即失败）。
  - **Git diff 配色改用 GitHub（Primer）**：此前用 macOS 系统绿/红混到 14%，既不像 macOS 也不像 GitHub。浅色用 Primer 实色（`#e6ffec`/`#ccffd8`/`#abf2bc`、`#ffebe9`/`#ffd7d5`/`#fdb8c0`），深色用 Primer 的 alpha 叠加值以适配面板底色；行号列按 GitHub 规则使用更重的底色（必须写组合选择器，否则 `.d2h-ins` 与行号规则同权重、后者被覆盖）。仅限 diff 区域，其他状态色一律不变。
  - **必须走 diff2html 自己的 `--d2h-*` 变量，而不是覆盖它的规则**：只覆盖 `.d2h-ins` / `.d2h-del` 只换掉了两种行底色，库自带的浅色调色板在明暗两种主题下都继续透出——`@@` 行仍是 Bootstrap 蓝（`#f8fafd` / 边框 `#d5e4f2`）、修改行仍是芥末黄 `d2h-change`（`#fdf2d0`）、行号列仍是 `--d2h-bg-color: #fff`、增删行**边框**仍是库里的旧绿旧红（`#b4e2b4` / `#e9aeae`）。这些残留才是「不伦不类」的真正来源。现在明暗两套 `--d2h-*` 变量全部指向同一组随主题切换的 `--diff-*` token，行底色、词高亮、边框、行号、hunk、change、占位块与 `+n/-n` 统计标签一并覆盖，且无论 diff2html 输出哪个 color-scheme class（默认 `light`）都成立。
  - **文件树行操作改为悬浮**：`@` 与复制按钮不再常驻占位，静止时文件名与体积占满整行宽度，不再在右侧留出大片空白；仅在按钮显示时行内让位。
  - 变更列表每行恒为单行：此前行上的 `flex-wrap: wrap` 会按路径按钮的**假定尺寸**（内容宽度）断行，长路径本身就已超出行宽，`min-width: 0` 与 `flex-shrink` 根本没有机会生效，`@` 悬浮操作被挤到第二行。现在行不换行（仅附件列表保留换行，因其内联预览是 `flex: 0 0 100%` 的独立行），`@` 改为绝对定位悬浮在行右端：静止时路径占满整行宽度，仅在按钮显示时行内让出空间，只移动省略号位置而不推动文字。右键菜单继续提供提及、查看 diff、复制路径与在访达中显示。
- 守卫：`chat-ui.test.mjs` 新增三栏下限、JS 宽度钳制与视口绑定、按栏百分比边距（并对 `.messages`/`.composer-wrap`/`.chat-title-name` 中的 `vw` 做 `doesNotMatch`）、Header flex 约定、1000px 抽屉断点。`svelte-check` 0 错误 0 警告，`vite build` 通过，桌面端全量测试（106 Chat UI + 71 单测 + 26 Rust）通过；并在浏览器中对真实样式表实测：1280px 窗口请求 720px 面板时网格解析为 `260px 460px 560px`，标题截断且与操作区保持 12px 间距。

### Chat 来源微标签与上下文消息时间（已完成，P2）

- Chat Header 移除抢眼的大圆形首字母头像，改为低对比的 `# + 来源首字母` 微标签；Web、Telegram、飞书、QQ、微信与 Project 均可区分，完整来源名通过无障碍标签和悬停提示保留。
- Header 进一步统一为 18px 高的 `# W / 标题` 序列：标签、斜杠与标题使用同一字号并严格上下居中，标题不再以更大、更粗字体抢占视觉焦点。
- 左侧四个主要入口压缩为 30px 行高，改用统一线性语义图标，并移除入口与对话树之间多余的横向分隔线。
- 将侧栏内容起点移到完整 60px 原生标题栏拖动区下方，「新对话」不再被交通灯/拖动遮罩覆盖；Chat 顶栏整条空白与被动标题区域均可拖动窗口，右侧操作按钮继续正常点击。
- 外部渠道只读会话移除消息区顶部重复的「来自渠道」提示，将来源与只读状态合并到底部一行，例如「来自 Telegram · 此会话在桌面端为只读」。
- 侧栏「对话」与「项目」升级为同一吸顶槽位中的一级分组标题：会话滚动时「对话」固定，进入项目内容后「项目」自然将其顶替，任何时刻只固定一个标题；标题保持透明并使用 14px 背景模糊遮蔽滚过的文字，不新增可见色块；降低透明度模式回退到既有侧栏表面。
- 普通 Chat 不再在标题中重复渠道名，Project Chat 仍保留有意义的「项目 / Session」标题层级。
- 普通 Chat 与 Project Chat 共用上下文时间格式：今天显示时间，昨天显示「昨天 + 时间」，更早显示本地化日期与时间，跨年时补充年份。
- 新增日期边界单测与 Chat Header 结构守卫；`svelte-check` 0 错误 0 警告，Desktop Chat UI 测试 104/104 通过。

### Desktop 全局下拉框统一为设计规范 Select（已完成，P1）

- 将 Desktop 设置、Project 设置和首次引导中的 55 个原生选择框全部迁移到共享 `SelectControl`；Trace、用量、日志、Skills、Providers、Plugins、Agents、Channels、任务及媒体工具等页面共用一套交互。
- `SelectControl` 改为 Bits UI listbox：统一 40px 触发器、12px 浮层、当前项勾选、长列表滚动、方向键/Enter/Escape、typeahead、禁用态、焦点环与中英/明暗主题。
- 时间与数字输入继续保留原生直接输入语义；机器守卫要求全部 Desktop Svelte 源码不得重新出现 `<select>`。

### 输入区模型与推理强度合并菜单（已完成，P2）

- Chat 与 Project Chat 的两个原生下拉合并为一个紧凑摘要按钮，直接显示「模型 · 强度」，减少输入区横向占用。
- 自定义浮层先展示模型和推理强度两项摘要，再在同一浮层内进入对应选项列表；长模型列表内部滚动，不再使用系统菜单。
- 支持点击外部关闭、Escape 返回/关闭、方向键循环、当前项勾选、键盘焦点与中英/明暗主题；保留原有 Session 级模型持久化和推理强度运行逻辑。
- 机器守卫禁止共享输入区重新出现原生 `<select>`，并要求模型与强度选项共用可访问菜单。
...
### App 数据页筛选控件不再叠加（已完成，P0）

- 用量、Trace 与服务日志共用的筛选网格现在让 Search / Select 包装器严格填满各自列，不再以 260px 固定宽度越过网格并盖住相邻控件。
- SearchField 内层输入不再重复继承外层的边框、背景与焦点环，恢复为单一清晰的搜索控件；相邻控件统一保留 12px 可见间距。
- 英文标题与操作按钮不再把 860×620 窄窗中的筛选网格撑出卡片；结构守卫覆盖列宽、单边框和窄屏收缩约束。

### App 服务日志逐行详情检查器（已完成，P1）

- 服务日志表格支持整行与明确按钮打开详情弹窗；长 Run ID 在列表中以首尾缩略形式展示，完整关联标识保留在详情中。
- 结构化 `[mom-t]` 记录在弹窗中格式化为缩进 JSON，旧日志和第三方文本原样展示；详情支持复制，并适配键盘、中英、明暗主题与窄屏卡片。

### App 服务日志运行中自动轮转（已完成，P1）

- Desktop Supervisor 使用成熟的 Rust `file-rotate` writer 接管服务 stdout/stderr：活动日志达到 20 MiB 后自动归档为 `.1`，继续写入新的活动文件，无需等待 App 或服务重启。
- 保留最近 5 个历史文件，超过数量的最旧归档自动删除；stdout 与 stderr 按完整日志行写入同一个线程安全 writer，正常结构化记录不会跨文件拆分。
- 服务停止、崩溃或重启时会等待两个日志泵完成落盘，防止新旧进程同时操作同一组归档。SQLite Trace 与其他 JSONL 业务/审计存储不参与通用轮转。
...
- **恢复的回合再次申请审批时，卡片弹不出来**：审批卡只走 SSE 推送，而"被审批唤醒"的那次 resume 在后台跑、根本没有流，于是它自己发起的第二次审批发给了空气 —— 用户只看到 Agent 说"去卡片里点批准"，但卡片从来没出现，只能手打「批准」。现在前端可以主动查询本会话的待审批（`list_pending`），在等待 resume 期间轮询、并在任意回合结束且无卡片时补查一次，并按 pitfall #3 做了会话归属校验。
- **写文件返回的路径 bash 用不了**：write 工具原样回显请求路径（"Wrote 197 bytes to release_notes.md"），但 scratch 写入落在 scratch 根下、而 `bash` 在项目根跑，Agent 把这个字符串交给 `gh release create --notes-file` 就是 "no such file or directory"。现在凡是无法从运行 cwd 解析的写入，都返回真实位置。（这条是被上面的 cwd 修复暴露出来的：以前两者恰好是同一个目录，而那正是 git 跑不了的原因。）
...
### 项目文件面板：大文件预览、媒体流式播放与浏览器级交互（已完成，P0+P1）

- **现象**：右侧文件面板"只能看不好用"—— 稍大一点的文件就完全打不开，视频拖不动进度条，PDF/HEIC 根本没有预览。
- **根因一（256KB 是终点不是截断）**：`readProjectFile` 在 `stat.size > 256KB` 时一个字节都不读，直接返回 `oversized`，而 `maxBytes` 又被夹死在同一个上限，上层没有"我要更多"的通道。现在改为窗口式读取：单次 512KB，用服务端返回的 `byteOffset + byteLength` 作为下一次的 `offset` 往后翻，文本上限提到 16MB；查看器按需加载下一段并显示"已加载 / 总大小"。
- **根因二（窗口边界会切坏中文）**：按字节切窗口必然从多字节序列中间断开，每一页的首尾汉字都会变成 ``。服务端把窗口收敛到完整 UTF-8 字符边界，并回报实际消费的字节数，客户端不可能算出一个切坏字符的偏移（对应 pitfall #7）。
- **根因三（UTF-16 被误判为二进制）**：`looksBinary` 只要前 8KB 出现 NUL 就判二进制，而 UTF-16 文件遍地是 NUL。现在先看 BOM、再看无 BOM UTF-16 的 NUL 位置模式、最后看控制字符占比，UTF-16LE/BE 会被正确解码。
- **根因四（媒体全量读进内存、不支持 Range）**：raw 端点用 `fs.readFile` 把整个文件读进内存，响应头没有 `Accept-Ranges`，所以 `<video>` 无法 seek、大文件直接把服务进程内存打爆；而且还带了一小时的强缓存，Agent 改写文件后仍然返回旧内容。项目 raw 路由与 `/api/web/files` 现在共用同一个 `streamFileWithRange`（206 / 416 / 304 + 弱 ETag + `no-cache`），会话附件也从"在 JS 里拼 Blob"改为直接指向流式 URL。
- **格式补齐**：MIME 表补上 heic/heif/avif/ico/apng/mkv/ogv/mpeg/3gp 与 `application/pdf`；PDF 用 embed 渲染，SVG 默认渲染并提供"看源码"切换，仍然无法预览的类型给出"用默认应用打开"。
- **交互补齐（P1）**：文件树支持方向键导航（← 折叠/上移，→ 展开/下移，Enter/Space 打开，⌘↓ 用默认应用打开）；树行、变更行和查看器都有右键菜单（打开 / 查看改动 / 引用到对话 / 复制路径 / 在访达中显示 / 用默认应用打开）；静态路径行换成可点击面包屑；图片查看器支持滚轮与触控板缩放、拖拽平移、适应窗口 / 1:1 与透明棋盘底；文件列表可整体收起，把 280–720px 的窄栏完全让给查看器。
- **跟随 Agent 改动**：可开关的跟随模式，Agent 每写一个文件就打开它的 diff 并在树中定位；打开旧会话时只静默接管既有历史，不会突然跳转。
- **安全边界**：「在访达中显示 / 用默认应用打开」由本地服务解析绝对路径并调用 `open`（数组参数、不过 shell），绝对路径不回传给 WebView（对应 pitfall #5）。
- **新增机器守卫**：`ph-finder-logo` 这个不存在的图标名上线后只会显示成一个空框、不报任何错 —— 与 pitfall #4 的 CSS token 是同一类静默失败。现在有测试校验 Desktop UI 里所有 `ph-*` 图标名都存在于已安装的 Phosphor 图标集。另外原本就存在的 raw 路由测试没有被任何 npm script 引用，已挂进 `test:projects`。
- **验证**：`svelte-check` 0 error / 0 warning、服务端与 Desktop 生产构建、46 项项目测试（新增 5 项：分页、CJK 窗口边界、UTF-16、16MB 上限、206/416 Range）、98 项 Desktop UI 守卫（新增 1 项图标守卫）。
...
### App 项目文件面板与对话打通（已完成，P2）

- **改了什么**：文件面板此前和对话是两个互不知情的世界 —— 面板不知道 Agent 改了什么，对话也拿不到面板里的文件。这一刀把两边接上。
- **文件引用进输入框**：目录树每一行、搜索结果、以及查看器标签页都有 `@` 动作，点击把 `@路径`（内容搜索结果则是 `@路径:行号`）追加到项目对话的输入框，前后各留一个空格。面板和输入框在组件树上是兄弟节点，因此用一个普通 Svelte store 连接 —— `ProjectChat` 是 legacy `$:` 组件，无法跟踪其它模块的 runes `$state`。
- **Agent 改过的文件会亮起来**：本次会话中 Agent 写入过的文件，在目录树上带一个圆点并加粗文件名。正在进行的这一轮的工具调用也算数，不用等回合结束。
- **变更页支持"本次会话"范围**：变更页默认只列出当前会话中 Agent 写入过的文件，一键切回完整 `git status`。标签上的角标优先显示本次会话的数量。
- **改动来源是结构化字段而不是解析文案**：工具调用现在把自己参数里的路径记录到 activity 的 `paths` / `mutates` 字段（`src/lib/server/app/toolFilePaths.ts`），而不是反过来去解析给人看的 label —— label 会本地化、会截断、还会加显示名前缀，按它做解析改一次措辞就崩。绝对路径、`~`、`..` 越界路径直接丢弃，因为它们永远匹配不上 `git status` 的条目，记下来只会让面板显示一个打不开的"变更文件"。
- **为什么不用工作区快照**：改动集来自 transcript 而不是"会话开始时的 git 快照"，因此重启后仍然正确，来回切换会话也不会串。
- **验证**：`svelte-check` 0 error / 0 warning、两个生产构建、96 项 Desktop UI 守卫（新增 2 项）、24 项项目/activity 服务端测试、220 项 desktop-chat 套件、针对新字段的"保存 → 新建 store → 读取"往返守卫（对应 pitfall #10），以及对改动文件跑 `tsc` —— 仓库既有的 170 个错误数量未变，改动文件中为 0。

---
## 2026-07-27

### App 项目文件面板升级为可用的代码浏览器（已完成，P0 + P1）
- **改了什么**：右侧"项目文件"面板从"点一层换一屏、一次只能看一个文件、裸 `<pre>` 预览"的只读列表，变成带真实目录树、多标签页查看器和搜索的代码浏览面板。
- **真正的目录树**：目录就地展开/折叠，展开状态按路径持久保存，刷新单个目录不会把整棵树收起来。每层懒加载、分别缓存、分别分页；顶部提供"折叠全部"。
- **多文件标签页**：打开文件不再关闭已打开的文件。查看器位于树的下方，最多 12 个标签，支持单个关闭、全部关闭，中间的分隔条可拖动调整上下比例（位置写入 localStorage）。
- **代码查看器**：`highlight.js` 语法高亮（按行切分时会重开跨行的 span，块注释和模板字符串不会串色）、行号、软换行开关、文件内查找（`⌘F`，带匹配计数与上/下一个）、点行号复制 `路径:行号`。超长文件按 2000 行分块渲染。
- **搜索**：`⌘P` 按文件名模糊查找，`⌘⇧F` 全文搜索（带行号与高亮匹配位置）。选中结果会自动展开目录树到该文件并把查看器滚动到对应行。打分按字符而非按词，因此中文目录名和中文正文都能匹配——按空格分词会把 CJK 整段压成一个 token。
- **文件变更实时同步**：面板订阅变更流，只刷新真正受影响的目录层和已打开的标签；类似 `git checkout` 的大批量变更会退化为整体刷新。刷新按钮上的小圆点表示流是活的；平台无法递归监听时自动退回手动刷新。
- **面板宽度可拖拽**：280–720 px，复用侧边栏的 direct-manipulation 手感并持久化，替代原来写死的 `27%`。
- **变更页**：diff 以查看器标签打开，支持并排对比切换；标签上显示脏文件数量，树中变更过的文件会标记出来。
- **后端新增**：`src/lib/server/projects/search.ts`（有界遍历：2 万文件 / 16 层 / 5 秒预算，遵守根目录**与嵌套** `.gitignore` 并跳过 vendor 目录，字面量内容匹配，二进制嗅探，不跟随越界软链）与 `watcher.ts`（递归 `fs.watch`，按真实根路径引用计数复用，250ms 去抖，过滤编辑器临时文件）。两者都落在既有的 `/api/settings/projects*` Tauri HTTP scope 内，无需改权限。
- **仍然保持只读**，也没有引入编辑器内核；新的遍历不调用外部命令，因此针对外部目录加固过的 `runGit` 完全未动。
- **验证**：`svelte-check` 0 error / 0 warning、两个生产构建、94 项 Desktop UI 守卫（新增 5 项覆盖树展开、多标签、搜索时序、watcher）、新增 `test:projects` 脚本下的 16 项项目服务端测试，以及一次真实冷启动走查：目录树、中英文文件名搜索、中文全文搜索的行号与偏移、SSE 变更流对真实写入的反应、以及 `../` 越界仍被拒绝。

### App AI 服务商改为内联工作台（已完成，Issue #20 收尾）
- **改了什么**：右侧详情区从"只读摘要卡 + 编辑按钮打开大弹窗"变成**实时编辑面板**。选中服务商即载入草稿，API Key、API 地址、模型清单、启停都在原地改，不再进出弹窗。弹窗只保留两个真正需要聚焦的场景：单模型编辑、拉取模型列表。
- **左侧列表**：每个服务商有按 id 生成的固定色首字母标记、名称、模型数与 ON/OFF 胶囊；顶部搜索 + 内置/自建切换 + 已启用优先排序，底部固定"新增自建服务商"。新建的服务商以"未保存"草稿行直接出现在列表里，选中态、脏数据判断和详情区走同一条代码路径。
- **API Key**：输入框、显示/隐藏切换、"检测"按钮合成一个控件；API 地址下方实时显示真正会请求的完整 URL。
- **模型清单**：按模型系列（前两段）分组、可折叠，每行显示 ID、能力图标、上下文规模、启用开关、编辑与移除。拉取模型弹窗用同样的分组，支持 全部 / 未添加 / 已添加 过滤和整组添加。
- **需要登录的服务商**：未连接时以居中头像 + 登录按钮作为主动作；已连接折叠为一行状态卡（测试连通性 / 退出登录）。
- **未保存保护**：保存后面板停留在当前服务商而不是关闭；带着未保存修改切换服务商会先询问，不再静默丢弃。
- **拉取模型弹窗可滚动**：修复模型多时（51 个）所有行被压成细条、内容看不见的问题。弹窗自身固定高度，模型列表是弹窗内唯一的滚动容器，每行保持完整高度，超出部分滚动查看；已加回归守卫。
- **宽度对齐**：工作台使用共享的设置列宽 `--settings-col`，与"全局选择"等其它设置卡片左右边缘完全对齐，不再比页面其它组件宽。
- **颜色未改**：全部沿用现有 macOS 语义色板与 token，明暗主题各验证一次。
- **验证**：`svelte-check` 0 error / 0 warning、生产 `vite build`、87 项 Desktop UI 守卫 + reactivity/http-scope 守卫、明暗主题渲染检查。本次未改 Web `/settings/ai/providers`。
...
- 设置表单中的文本、数字、时间和下拉控件统一使用 `DESIGN.md` 默认 input token 的 40px 高度，不再由不同 HTML 控件的原生内边距自行决定大小。
- 记忆反思通知行移除冲突的 `settings-row settings-field` 混用，时间框不再被整行布局拉高；所有普通设置字段继续使用共享主题边框、圆角、焦点环和明暗主题 token。
- 新增共享 `NativeTimeInput`，一次覆盖记忆反思、每日素材和自动任务时间；点击整个原生 `input[type=time]` 会在 WebView 支持时调用系统 picker，不支持时仍可键盘输入，不引入第三方时间组件。
- 验证：Desktop UI 81/81、设置/临时数据库重启 13/13、Svelte 0 错误 0 警告、production build、860×620 冷打开与插件页切换无横向溢出。

### Desktop 用量与 Trace 紧凑筛选条（已完成）
- 用量页把时间范围、模型、Bot、渠道放进独立的紧凑筛选行；时间范围继续使用今天、昨天、最近 7 天、最近 30 天的下拉选择，选择即按原逻辑刷新。清空与图标刷新位于上方轻量标题行，不再和四个控件争抢 720px 内容宽度。
- Trace 默认只展示时间、事件类型、Bot、渠道四个高频维度；Chat / Session / Run ID 与读取上限移入可键盘展开的“更多筛选”，并用徽标提示已填写的高级条件。折叠行改为透明低权重表面，清空使用 tertiary、刷新使用图标，只有“应用筛选”保持主按钮。原有批量提交语义与全部查询能力不变。
- 两页将生效日期、时区和更新时间压缩为安静的元数据行，并在筛选卡与 KPI 之间统一留出 24px 层级间距；共享样式支持中英、明暗主题和 860×620 重排。
- 验证：Desktop UI 80/80、Svelte 0 错误 0 警告、production build、860×620 冷打开/页面切换无横向溢出。浏览器预览缺少 Tauri 服务桥接，因此填充数据态由结构回归与构建覆盖。

### 每日素材卡片可直接配置通知目标（已完成）
- Desktop「插件 → 每日素材」展开区新增记忆任务通知目标下拉框，直接复用记忆后端设置中的已授权 Telegram/飞书目标；两处编辑同一份结构化设置，不引入互相冲突的第二目标。
- 每日素材与记忆反思继续保留各自通知开关；只要任一通知开启，两张卡片中的共享目标选择器均可用。中英文提示不再依赖“上方”这一折叠布局位置。
- 结构回归要求两张卡片都能编辑共享目标；验证 Desktop UI 80/80、授权/设置/重启聚焦测试 27/27、Svelte 0 错误 0 警告和 Desktop production build。
...
- 替换了深色主题中的 Geist/Vercel 配色系统为新的低对比度 OKLCH 配色方案，解决深色主题下对比度过强的问题。
- 更新了 `src/styles/theme.css` 中的 `.dark` 变量，只修改深色主题色，浅色主题色保持不变。
- 在 `design.dark.md` 中同步记录并归档了深色主题的全新 OKLCH 颜色及阴影变量配置，避免以后使用或修改错误。

### Desktop 展开 Session 列表紧凑化（已完成）
- Chat 左侧各渠道展开后的 Session 行统一从 40px 收敛到 DESIGN 紧凑控件档 32px，标题与时间统一使用 `label-12` 的 12px/16px 字体节奏，行内间距使用 4px 基础网格。
- 共享 `ConversationRow` 一次覆盖 Web、Telegram、飞书、QQ、微信及 Project Session，保留选中、状态、重命名、删除和只读行为；机器守卫同时锁定 DESIGN 来源与最终尺寸。

### Desktop 设置窗口复用、内置 AI 服务商配置与命令清理（已完成）
- Settings 原生窗口关闭时改为隐藏并复用，不再销毁已注册窗口；从 Chat、菜单或其他入口第二次打开设置仍可正常显示。
- 内置服务商使用独立精简编辑器，只保留 API Key、启用状态、默认模型与模型 ID 清单；不再展示自建服务商的协议、Base URL、Thinking、能力标签、角色和上下文窗口表单。大量模型时仅弹窗正文滚动，标题与保存操作持续可见。
- 内置服务商列表的绿色状态改为读取已保存的启用配置，不再因“属于内置目录”就全部显示激活；详情同时显示 API Key 是否配置。内置配置不会切换到自建模式，自建服务商仍要求独立 ID、Base URL 与 API Key。
- 设置页保存条固定贴住右侧内容区底边，滚动内容预留等高空间，不再随短内容悬浮在页面中部。
- `/login`、`/logout` 已从 Web/Desktop 补全、Telegram 注册以及 Web/共享 Channel 执行入口移除；微信自身的渠道登录流程不受影响。
- 机器守卫覆盖 Settings 关闭策略、内置服务商编辑映射/精简结构、真实启用状态、弹窗滚动链、固定保存条、共享命令建议、Telegram 命令目录与共享命令执行回退。

### Desktop Chat Header 共享转录搜索（已完成）
- 普通 Chat 与 Project Chat 的 Header 共用同一个转录搜索组件；展开后搜索框留在 Header 正常 flex 布局内，不再以绝对定位覆盖标题、文件或设置按钮，窄窗口会由标题主动让出空间。
- 搜索按消息实际渲染文本匹配：附件占位和 Assistant 错误回退使用用户真正看到的本地化内容，缺少可导航 ID 的记录不会进入结果；会话或结果数量变化时当前序号自动收敛到有效范围。
...
- Chat 拖拽热区与悬浮面板可见右边缘对齐，并从当前侧栏宽度开始直接操控，避免首次拖动跳回最小值；键盘左右调整、窄窗口和文件面板隐藏规则继续生效。
- 材质随明暗/失焦状态适配；减少透明度与低性能模式使用不透明实色并关闭 blur/阴影。真实渲染验证覆盖 1200×800、最低支持的 860×620、浅色/深色、低性能降级和 Chat 键盘 resize，均无横向溢出。
...
- 验证：Desktop UI 60/60、Svelte 0 错误 0 警告、Desktop production build 通过；当前沙箱禁止监听本地端口，未执行浏览器视觉巡检。
...
### Desktop Agent City 楼层悬浮信息与楼体运行态（已完成）
- Three.js 城市首次打开不再常驻显示任何楼层圆点、文字或面板。鼠标命中 Global 总部或某一楼层本体时，才在该位置展示唯一一张 Agent、任务、渠道、模型路由和 Sub-agent 信息卡；移动到其他楼层会切换，离开楼体即隐藏。
- `working` 状态完全绘制在对应玩偶屋楼层的外墙、屋顶和底座轮廓上：保留低亮外框，并让一段长而高亮的场景内虚线持续绕楼体循环，不再依赖 DOM 悬浮框。减少动态时虚线固定为静态亮边；2D 降级视图继续保留等价的键盘悬浮/焦点详情与静态状态对比。
- 验证：Agent City 投影/场景测试 9/9、Desktop UI 80/80、Svelte diagnostics 0 错误 0 警告、Desktop production build 通过；真实冷启动交互走查待具备原生窗口辅助功能/屏幕捕获权限的本机完成。
...
- Agent 信息仅在鼠标命中对应 Three.js 楼层时显示一张详情卡；2D 降级视图保留同等详情与键盘语义。
...
- **验证**：mory 包测试 179/179 通过（含新增 tokenizer 单测 11 项：「短版」命中、「调研」bigram 通道、虚词零分、中英混合、排序断言）；宿主新增 `src/lib/server/memory/classifier.test.ts` 2/2 通过；desktopMemory 与 sessions store 回归 7/7 通过。详见 `docs/archive/requirements/memory-improvement-plan.md` T1a。
...
- 使用真实 Chat、Automations、Skills 截图对照 `DESIGN.md` 完成组合 UX/可访问性审计，证据与报告保存在 `docs/work/audits/chat-workspace-2026-07-04/`。
