...
|---|---|---|---|
| Observability | Per-turn call trace and HTML sharing | 部分交付 | Call Trace plugin, scoped report queries, Desktop HTML viewer and R2 publishing adapter are implemented; shared query, rendering and restart tests exist. Mini App is deferred; external Agent internals and live R2 delivery are not fully verified. |
