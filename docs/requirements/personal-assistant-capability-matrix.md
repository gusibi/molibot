...
| Planning | Session-generated plan board (list, edit, control, cross-session continue) | 部分交付 | Plans are saved on generation as Durable Execution records (stable `planId`, two-layer tasks) and appear in a desktop "Plans" workspace (search/status filters, in-flow detail, rework/delete). Execution happens as an ordinary Session turn in the source Session: streaming thinking/tools/output and standard chat approval cards, with progress mirrored into the Durable record (board read model); the right inspector is read-only for plans. Evidence: `plans/*.test.ts` + durable/session-plan suites pass, desktop svelte-check/build and mjs guards pass. Gap: board has no create/execute entry of its own (continue opens the source Session), project/external-session plans only open the conversation, and no live cold-start/release walkthrough yet. |
