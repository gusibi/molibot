...
|---|---|---|---|
| Observability | Per-turn call trace and HTML sharing | 部分交付 | Call Trace plugin, scoped report queries, approval outcome recording, Desktop HTML drawer and R2 publishing adapter are implemented; shared query, rendering and restart tests exist. Mini App is deferred; external Agent internals and live R2 delivery are not fully verified. |
...
| Planning | Session-generated plan board (list, edit, control, cross-session continue) | 部分交付 | A proposed plan is saved immediately as a Durable Execution (idempotent `planId`, two-layer tasks) on both stream and chat paths and linked on the Session plan; accepting it applies pre-approval edits as a new version and starts it through the Durable runtime instead of resuming the chat turn; the desktop "Plans" workspace lists/searches/filters plans and offers start/pause/resume/cancel/delete plus append-rework. Evidence: `plans/*.test.ts` 12/12, durable + session-plan suites 42/42, desktop svelte-check/build and mjs guards pass. Gap: the board does not yet render confirm/approval/evidence interactions (the sidebar Durable inspector does), explicit `setAttemptContextSession` claiming/handover history, first-approval permission selection, and live events (polling only). |
