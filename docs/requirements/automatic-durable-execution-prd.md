...
- Keep source-of-truth state in a dedicated injectable SQLite store (`<dbDir>/durable-execution.sqlite`) under the existing centralized database boundary, alongside the existing per-domain databases. Plans, steps, criteria, side effects and decisions are rows, never Markdown or JSON files: only a database gives optimistic version CAS, atomic intent/receipt writes, owner-scoped indexes and a trustworthy read after a crash. Tests always use a temporary database. No state is written into a user's Project root.
