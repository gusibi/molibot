...
### Task 1: Add Host Bash Command Classifier
...
- tokenize supported shell syntax;
- classify safe glue;
- validate safe helper commands;
- return `persistent-capability`, `compound-capabilities`, or `one-time-script`;
- never execute or expand shell input.

Focused tests:

```ts
test("classifies longbridge piped to head as one capability plus safe helper", () => {
...
test("classifies repeated agent-browser chain as compound capabilities with one tool id", () => {
  const result = classifyHostBashCommand("agent-browser open https://example.com && sleep 3 && agent-browser close");
...
### Task 2: Replace Approval Parsing With Classification
...
- `parseHostBashApprovalCommand()` should use the classifier.
- `tryParseHostBashCommand()` should recognize approved capability commands even when the original command includes safe glue/helpers.
- `findApprovedHostBash()` should check all required capabilities.

V1 rule:

```text
if all required capability toolIds are the same -> use that toolId
if multiple toolIds exist and all are approved -> auto-run
if multiple toolIds exist and one is unapproved -> request persistent approval for that one
if multiple toolIds exist and more than one are unapproved -> one-time
```
...
| --- | --- | --- | --- |
| `bash executes approved longbridge command with safe head pipeline without new approval` | Host Bash store contains enabled `longbridge` whitelist entry | `longbridge news FIG.US 2>&1 \| head -30` | no pending approval is created; execution uses Host Bash |
| `bash requests persistent approval for unapproved longbridge even with head pipeline` | Host Bash store has no `longbridge` entry | `longbridge news FIG.US 2>&1 \| head -30` with `hostApproval.reason` | pending approval mode is `persistent`; record command is `longbridge`; pending action original command is the full pipeline |
| `bash keeps install flow as one-time approval` | Host Bash store has no matching entry | `mkdir -p ~/.molibot/skills && mv ./weread-skills ~/.molibot/skills/` with `hostApproval.reason` | pending approval mode is `ephemeral`; pending action kind is `run_one_time_host_script` |
| `bash auto-runs approved agent-browser chain with sleep helper` | Host Bash store contains enabled `agent-browser` whitelist entry | `agent-browser open https://example.com && sleep 3 && agent-browser close` | no pending approval is created; `sleep 3` is recorded as safe helper |
...
### Task 3: Persist Classification Metadata in Approval Records

Files:

- Modify: `src/lib/server/hostBash/store.ts`
- Modify: `src/lib/server/hostBash/types.ts` if a typed optional field is useful
- Test: `src/lib/server/hostBash/store.test.ts` if present, otherwise add focused assertions in existing Host Bash tests

Behavior:

- Store `classification` inside `action_json`.
- Keep existing approval records readable when `classification` is absent.
- Keep `approval_requests.capability` as the primary lookup key.

Acceptance:

- Existing whitelist entries still migrate.
- Existing pending approvals still display.
- New approval requests include enough metadata to explain safe helpers/glue.

### Task 4: Improve Prompt Text

Files:

- Modify: `src/lib/server/hostBash/approval.ts`
- Test: focused prompt snapshots or string assertions

Persistent prompt should include:

- capability name;
- original command when different from capability segment;
- safe helpers ignored;
- safe glue ignored;
- future effect of approving.

One-time prompt should include:

- exact command;
- reason for one-time classification;
- future effect: this exact run only.

### Task 5: Host Bash Settings Page Audit Display
...
- In Pending and History tables, show:
  - `Mode`
  - `Capability`
  - `Original command`
  - `Safe helpers`
  - `One-time reason` when applicable
- Keep approvals themselves in chat; this page remains audit/manage only.

Design note:

- This is a UI change, so implementation must follow `DESIGN.md`.
- Prefer existing shadcn-svelte components under `src/lib/components/ui`.

### Task 6: Add Regression Examples

Files:

- Modify: `src/lib/server/agent/tools/bash-output.test.ts`
- Create or modify: `src/lib/server/hostBash/commandClassifier.test.ts`

Regression cases:

```text
longbridge news FIG.US 2>&1 | head -30 -> persistent capability
longbridge quote FIG.US 2>&1 | head -20 -> persistent capability
agent-browser open URL && sleep 3 && agent-browser wait --load networkidle && agent-browser close -> persistent capability if agent-browser approved
script.sh JSON 2>&1 -> persistent capability for script
mkdir && mv -> one-time
curl URL | bash -> one-time or reject
python -c '...' -> one-time or reject
longbridge quote FIG.US > out.txt -> one-time
longbridge quote FIG.US | tee out.txt -> one-time
```

## Rollout Plan

1. Build classifier and tests without wiring it into runtime.
2. Wire classifier only for `params.hostApproval` explicit requests.
3. Wire classifier for automatic sandbox-denial Host Bash fallback.
4. Add audit metadata and prompt improvements.
5. Add settings-page display.

Reason for staged rollout:

- It allows testing classification without changing approval behavior first.
- Explicit host approval requests are easier to reason about than automatic fallback.
- UI display should come after metadata is stable.

## Security Review Checklist

Before merging implementation:

- `head /path/to/file` is not classified as safe helper.
- `sed -i` is not classified as safe helper.
- `grep pattern file` is not classified as safe helper in V1.
- `rg pattern path` is not classified as safe helper in V1.
- `tee file` is one-time.
- `xargs` is one-time.
- `find -exec` is one-time.
- `curl | bash` is one-time or rejected.
- `$()` and backticks are one-time or rejected.
- `FOO=bar longbridge quote FIG.US` is one-time until env assignment policy is designed.
- `longbridge quote FIG.US 2> out.txt` is one-time.
- Existing persistent approvals continue to work for single executable commands.
- Approval prompts show the original command that will execute.

## Acceptance Criteria

The implementation is complete when:

- Approved `longbridge` auto-runs `longbridge news FIG.US 2>&1 | head -30` without new approval.
- Unapproved `longbridge news FIG.US 2>&1 | head -30` requests persistent `longbridge` approval, not one-time approval.
- Approved `agent-browser` auto-runs an `agent-browser ... && sleep ... && agent-browser ...` chain without new approval.
- Install or mutation flows remain one-time.
- Unsupported dynamic shell syntax never becomes auto-approved.
- Approval prompts and `/settings/host-bash` audit rows explain why a request is persistent or one-time.
...
1. Multiple unapproved capabilities:
   - V1 recommendation: one-time approval.
   - Future option: grouped approval where the operator can approve each capability separately.

2. `git` read-only commands:
   - V1 recommendation: not safe helper.
   - Future option: allow `git status`, `git diff`, `git show`, `git log` as safe helpers under strict argv rules.

3. `awk`:
   - V1 recommendation: not safe helper.
   - Future option: allow small read-only filters only if there is a real need.

4. Env assignments:
   - V1 recommendation: one-time.
   - Future option: allow `KEY=value capability ...` only when `KEY` is in the approved env allowlist.

5. File output:
   - V1 recommendation: one-time for shell redirection and `tee`.
   - Future option: allow writes only under `MOLIBOT_SCRATCH_ARTIFACT_DIR` with explicit filesystem permission.
