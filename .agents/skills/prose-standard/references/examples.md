...
**Over-trimmed:** "Mini App execution isolation."

**Balanced:** "Mini App server code runs in a separate process with its own fault domain; a hung handler is killed by the process deadline. See [ADR 0002](../../../../docs/adr/0002-untrusted-runtime-process-boundaries.md) for the boundary rationale."
...
**Balanced:** “Disposal aborts the run and waits for provider quiescence. See the owning ADR for ownership and race handling.”

**Over-detailed:** Repeating the ADR's promise choreography and rejected ownership models beside every disposer.
...
## Shipped features retain verification contracts

**Over-trimmed:** Deleting the entire verification section because the feature has already shipped.
...
**Balanced:** “Provider selection is cached for the plugin lifetime; installing or repairing a provider requires reload.” Keep ordinary cleanup in its TODO or the owning ADR.
