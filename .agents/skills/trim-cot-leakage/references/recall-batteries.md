...
Probes for [the taxonomy](../SKILL.md#taxonomy), tuned during the original corpus purge. Every hit needs semantic judgment — the batteries over-match by design, and they under-match by nature: each review round of the purge found cases no battery caught, so pair them with an unpatterned read of the densest prose in scope.

## Invocation rules

- Add `--hidden --glob '!.git/**'` so `.agents/` and dot-directories are searched; ripgrep skips them by default and the original purge's biggest miss risk was design documents and ADRs.
- Exclusions go last so a later include cannot re-admit them: `--glob '!node_modules/**' --glob '!build/**' --glob '!dist/**' --glob '!.svelte-kit/**' --glob '!.tmp/**' --glob '!.test-tmp/**' --glob '!docs/archive/**' --glob '!.agents/skills/trim-cot-leakage/**'` (the skill's own files quote leaked wording as calibration), plus recorded fixture and snapshot directories in scope.
...
- **"This PR" in process docs** — documentation *about* PR workflow ("the PR body should…", templates, this repo's process documents) legitimately says "PR"; the ban is on a doc adopting one PR's vantage about the code.
...
- **ADR context/consequences sections** — "rejected" inside an ADR's decision context is the sanctioned home, not review choreography.
