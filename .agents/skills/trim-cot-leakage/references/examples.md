...
**Leaked:** "Personal facts default to the owner namespace (decision 21)."

**Fixed:** "Personal facts default to the owner namespace — the canonical-namespace decision, owned by [ADR 0001](../../../../docs/adr/0001-memory-namespace-and-turn-retention.md)."
...
Both live facts survive — the current mechanism and the standing failure behavior — restated in the present. The bug's biography belongs to the PR and the `features.md`/`CHANGELOG.md` change record.
...
"This cut" / "v1" / "today" go stale the moment they merge. A historical stage name inside a `features.md`/`CHANGELOG.md` change record ("the first cut shipped X") is current-state-safe; the indexical form never is.
...
**Fixed (in an ADR's context/consequences):** "**Caching the resolved spec.** Rejected: the spec depends on per-call cwd, so a cache keyed by request would serve stale roots."
...
An unaided pass deleted this, reasoning that issue citations belong only in ADR documents. Wrong direction: issues resolve at HEAD from any surface, and "#N owns the follow-up" is the sanctioned home for deferred work in a README. What ADRs and change records additionally sanction is citing *merged PRs* as evidence.
...
**Keep (after fixing):** `// eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- the one-element literal guarantees index 0.`
