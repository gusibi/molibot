...
type ActiveLogin = {
  ...
  /** Pending pairing code entered by the user; sent on the next status poll. */
...
/** Return up to 10 most recently registered local bot tokens for QR pairing. */
...
/** Display the QR code in terminal, with a URL fallback. */
...
export async function startWeixinLoginWithQr(opts: {
  ...
  try {
    ...
    const apiBaseUrl = opts.apiBaseUrl || FIXED_BASE_URL;
    if (!apiBaseUrl) {
      return {
        message:
      ...
      switch (statusResponse.status) {
        ...
        case "need_verifycode": {
          const verifyPrompt = activeLogin.pendingVerifyCode
            ? "❌ 你输入的数字不匹配，请重新输入："
            : "输入手机微信显示的数字，以继续连接：";
          activeLogin.pendingVerifyCode = await readVerifyCodeFromStdin(verifyPrompt);
          continue;
        }
        case "expired": {
          qrRefreshCount++;
