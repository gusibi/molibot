/**
 * Streaming markdown filter for Weixin text delivery.
 *
 * Weixin can now render much of standard Markdown natively. Keep supported
 * constructs, and strip only the pieces that still cause degraded output.
 */
export class StreamingMarkdownFilter {
  ...
  private pumpSOL(eof: boolean): string {
    ...
    if (b[0] === "-" || b[0] === "*" || b[0] === "_") {
      ...
      if (j === b.length || b[j] === "\n") {
        let count = 0;
        for (let k = 0; k < j; k += 1) if (b[k] === ch) count++;
        if (count >= 3) {
          if (j < b.length) {
  ...
  private pumpInline(): string {
    ...
    switch (this.inl.type) {
      ...
      case "italic": {
        for (let j = 0; j < this.inl.acc.length; j += 1) {
          if (this.inl.acc[j] === "\n") {
