...
App 服务端代码是 owner 自装的**完全可信代码**，在每 App 独立子进程运行。这个边界能隔离退出、崩溃、OOM 和卡死，但 V1 **没有权限沙箱**：「App 只碰自己的 `data/`」是目录约定，只在 HTTP 路由边界强制（app-id 作用域、路径校验），不是对服务端代码的安全承诺。UI 另有 sandboxed iframe 隔离。
...
**所有来源一视同仁**：从 GitHub 或 ZIP 安装不会带来权限沙箱，那是在用你的权限运行别人的代码。安装器保证的只是更窄的事：压缩包写不出暂存目录（穿越条目、symlink、超大与 zip bomb 会被拒绝），manifest 必须先校验通过才会进入安装根，来源会被记录并在管理器里显示。已有子进程故障隔离，但签名和权限粒度尚未实现——在此之前只装自己写的或读过的 App。
